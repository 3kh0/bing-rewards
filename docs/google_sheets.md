...
2. You will first need to configure the consent screen, here
...
3. Go back to `Credentials` tab 
	...
	1. (Optional): Under the section `Authorized redirect URIs`, click `Add URI`. Add the following 2 URI's: `https://localhost/` and `http://localhost/`
	1. Click `CREATE`
...
4. Update JSON filename and path
	...
	- For the two steps above, via the command line, it would look something like this:
```sh
...
6. To summarize, the end result of the above is the following:
	- In the `config/` directory, there is a `google_sheets_credentials.json` which was downloaded from the google credentials page.
